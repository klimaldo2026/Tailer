...
client = genai.Client(api_key=os.getenv("GEMINI_API_KEY", "ВАШ_КЛЮЧ"))
...
def _build_history_text(history: list) -> str:
    """Превращает историю в текст для контекста"""
    if not history:
        return ""
    lines = []
...
def generate_daily_task(user_id: int) -> str:
    ...
    diary_summary = _summarize_diary(diary)
...
"""
    try:
        ...
        return f"Доброе утро! 💪 Сегодня: 30 мин активности + 2 литра воды + одно откладываемое дело. Ошибка: {e}"
...
"""
    try:
        return f"📊 *Итоги недели*\n\n{client.models.generate_content(model=MODEL, contents=prompt).text}"
    except:
...
"""
    try:
        return f"💡 {client.models.generate_content(model=MODEL, contents=prompt).text}"
    except:
